...
| 장 | 제목 | 핵심 키워드 |
|---|------|------------|
| 1장 | 왜 별도의 API 서버가 필요한가 | SSR vs SPA, 프론트-백 분리 |
| 2장 | CORS 설정 | Same-Origin Policy, Preflight |
| 3장 | Spring Security 설정 | SecurityFilterChain, 공개/인증 엔드포인트 |
| 4장 | JWT 인증 흐름 | Access Token, Refresh Token, Interceptor |
| 5장 | 공통 응답 구조 ApiResponse\<T\> | 래퍼 클래스, ErrorResponse, PageResponse |
...
운영(Production) 서버에서는 `application.yml`의 값을 실제 도메인으로 변경한다:
...
```bash
export CORS_ALLOWED_ORIGINS="https://www.62dangnyang.com"
```
