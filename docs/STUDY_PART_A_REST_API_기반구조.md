...
브라우저는 안전을 위해, 낯선 서버에 **"진짜 요청"을 보내기 전에 "나 이거 보내도 돼?" 하고 먼저 물어봅니다.** 이것이 바로 **사전 요청(Preflight)** 입니다.

**비유하자면:**
> 🙋‍♂️ **브라우저**: (똑똑) 저기요, 제가 `GET` 방식으로 `Authorization` 헤더 붙여서 요청 보내도 될까요? (OPTIONS 요청)
> 👮‍♂️ **서버**: 네, `GET`, `POST` 다 괜찮고, `Authorization` 헤더도 허용해 줄게요. (허용 응답)
> 🙋‍♂️ **브라우저**: 아하! 그럼 이제 진짜 데이터를 요청할게요. (실제 GET 요청)

**실제 HTTP 통신 과정:**

1. **1단계: 간보기 (OPTIONS 요청)**
   브라우저가 먼저 `OPTIONS`라는 메서드로 서버를 찔러봅니다.
   ```http
   OPTIONS /api/animals HTTP/1.1
...
2. **2단계: 허락 (서버 응답)**
   서버가 "이 출처(Origin)와 메서드는 허용한다"고 답장을 줍니다.
   ```http
   HTTP/1.1 200 OK
   Access-Control-Allow-Origin: http://localhost:5173
   Access-Control-Allow-Methods: GET, POST, ...
   Access-Control-Allow-Headers: Authorization
   Access-Control-Max-Age: 3600  (1시간 동안은 다시 안 물어봐도 돼)
   ```

3. **3단계: 본론 (실제 요청)**
   허락을 받은 브라우저는 그제서야 진짜 데이터를 요청합니다.
   ```http
