...
| 장 | 제목 | 핵심 키워드 |
|---|------|------------|
| 6장 | API 설계 원칙 — RESTful URL과 HTTP 메서드 | 리소스 설계, PUT vs PATCH |
| 7장 | Entity → DTO → Controller 흐름 | 계층 구조, 변환 패턴 |
| 8장 | 페이지네이션과 필터링 | Pageable, PageResponse, @RequestParam |
| 9장 | 인증이 필요한 API 개발 | 입양 신청, 권한별 분기, 상태 머신 |
| 10장 | 파일 업로드 API | Multipart, FormData, FileStorageService |
| 11장 | 프론트엔드 API 모듈 구조 분석 | 도메인별 분리, TypeScript 타입, 대응 관계 |
...
> **이 문서의 범위**
> 파트 B에서는 실제 API 엔드포인트 개발과 프론트엔드 연동 패턴을 다루었다.
> - 파트 A: 기반 구조 (CORS, Security, JWT, ApiResponse)
> - **파트 B: 실전 API 개발 (RESTful 설계, Entity↔DTO, 페이지네이션, 인증 API, 파일 업로드, 프론트 모듈)**
