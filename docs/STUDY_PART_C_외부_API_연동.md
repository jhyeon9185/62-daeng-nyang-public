...
| 장 | 제목 | 핵심 키워드 |
|---|------|------------|
| 1장 | 외부 API 연동이란 | 내부 API vs 외부 API, 공공데이터포털 |
| 2장 | API 키 발급과 설정 관리 | 환경 변수, @Value, application.yml |
| 3장 | WebClient로 외부 API 호출하기 | WebClient.Builder, UriComponentsBuilder, JSON 파싱 |
| 4장 | 데이터 동기화 로직 | Upsert, DTO→Entity 변환, 증분 동기화 |
| 5장 | 스케줄링 — 자동 동기화 | @Scheduled, cron, @ConditionalOnProperty |
| 6장 | 관리자 수동 동기화 API | Admin 엔드포인트, 동기화 이력 관리 |
