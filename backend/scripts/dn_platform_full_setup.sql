-- ============================================================
-- DN Platform 전체 환경 구축 (Database + Tables)
-- ============================================================
-- 이 스크립트는 다음 두 단계를 하나로 합친 통합 파일입니다:
-- 1. dn_platform 데이터베이스 생성 및 권한 설정
-- 2. 전체 테이블(15개) 및 인덱스 생성
--
-- [실행 방법]
-- MySQL CLI: mysql -u root -p < backend/scripts/dn_platform_full_setup.sql
-- ============================================================

-- ------------------------------------------------------------
-- Phase 1. 데이터베이스 생성 및 권한 설정
-- ------------------------------------------------------------

-- 1) 데이터베이스 생성
CREATE DATABASE IF NOT EXISTS dn_platform CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- 2) 전용 개발 계정 생성 (선택)
-- 필요 시 아래 'your_password'를 실제 사용할 비밀번호로 수정한 후 주석을 해제하여 사용하세요.
...
CREATE TABLE IF NOT EXISTS volunteer_recruitments (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    shelter_id BIGINT NOT NULL,
